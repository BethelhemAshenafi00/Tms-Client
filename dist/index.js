...
console.log(parseStudent({id: "STU-001", name: "Hana Tadesse"}));
...
const webDev = {
    status: "ACTIVE",
    enrolledCount: 28,
    startDate: polyfill_1.Temporal.PlainDate.from("2024-09-01"),
};
...
console.log((0, api_response_model_1.renderApiResponse)(studentRes, (s) => `${s.name} GPA: ${s.gpa ?? "N/A"}`));
const courseListRes = {
    status: "success",
    data: [
        {
...
console.log((0, api_response_model_1.renderApiResponse)(courseListRes, (courses) => courses.map((c) => c.title).join(", ")));
// Ex4
...
const daysUntilStart = today.until(courseStart).total({ unit: "day" });
